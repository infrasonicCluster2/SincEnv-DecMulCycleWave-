...
//curve[i]= 2/Math.sqrt(3 * Math.PI)*Math.exp(-26 * x * x)/-1; //  Gaussian curve borrowed and reworked from (Davies and Jonsson 2013)
//curve[i]= 2/Math.sqrt(3 * Math.PI)*Math.exp(-11 * x * x * x * x)/-1.5; // Quasi-Gaussian
...
//curve[i] = (-21) / (12 + 1000.0) * Math.PI * 13.7 + Math.abs(x) - 0.1 * Math.exp(-2 * x * x * x * x * x * x ) /-0.20 - 0.2; // Triangle
...
//curve[i] = x * -x * Math.abs(x * x * x * x * x / 0.999); // Expodec and expo attack 

//curve[i]= Math.sin(x * 2  * x* 2 * x * x * 6 ) / 2; // Bipolar Modulator 
 
//curve[i] =  Math.sin(x * x^x - 1.0)/2.0; // square 0 to 1  
}
...
//arbitraryWave[i] = 2 / Math.sqrt(3 * Math.PI) * Math.exp(-20 * i * i) / - 1; // Gaussian curve borrowed and reworked from (Davies and Jonsson 2013). Discussed in (Roads 2001,p140)

//arbitraryWave[i] = (Math.sin(Math.PI * x * 6) / ( Math.PI * x * -7 ));  // large sine cardinal   borrowed and reworked (night owl 2012). Discussed in (Roads 2001,p140)
//arbitraryWave[i] = (Math.sin(Math.PI * x * 30) / ( Math.PI * x * -43 ));// narrow sine cardinal   borrowed and reworked (night owl 2012)


//arbitraryWave[i] = (Math.sin(Math.PI * x * 1)/ 1.5); //Sine wave as Discussed in (Roads 2001,p140)
//arbitraryWave[i] = (Math.sin(Math.PI * x * 4)/1.5); // Multicycle sine Discussed in (Roads 2001,p140)
//arbitraryWave[i] = Math.sin(x * x * 11 ) / 2 ; // A variation on a sine wave
 
arbitraryWave[i]  =  Math.cos(Math.PI * x * 5.11) / ( x - 1.191 / 0.64 ); // Decaying Multicycle Sinusoid (Roads 2001,p.140)
//arbitraryWave[i] =   Math.cos(Math.PI * x * 5.11) / ( x - 1.191 / 0.64 ) * Math.cos(Math.PI * x * 5.11); //Multicycle Sinusoid 0 to 1
 
//arbitraryWave[i]  =  Math.sin(x^8  * x^2 * x * x); //Squares 

//arbitraryWave[i]  = Math.abs(Math.PI * (x  - -0.41)- 1) - 2.2 * Math.abs(x * 1.3) * Math.PI / 3 - 0.29 - x + x / 1.2 * Math.abs(x); // odd shape
}
